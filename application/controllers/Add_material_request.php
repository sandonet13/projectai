...
class Add_material_request extends MY_Controller {
    ...
    function index() {
        ...
        $view_data['login_id'] = $this->Users_model->login_user_id();
    ...
    function get_pdf() {
        ...
        if($this->Users_model->login_user_id() == 17){
        $list_data = $this->Add_material_request_model->get_details_approved()->result();
        }else{
        $list_data = $this->Add_material_request_model->get_details()->result();
        }
        $view_data['list_data'] = $list_data;
        
        $this->load->view("add_material_request/get_pdf", $view_data);
    ...
    function list_data() {
        ...
        $options = array("date_request" => $date_request, "category_id" => $category_id, "location_id" => $location_id, "project_id" => $project_id, "user_id" => $user_id, "status_id" => $status_id);
        if($this->Users_model->login_user_id() == 17){
        $list_data = $this->Add_material_request_model->get_details_approved($options)->result();
        }else{
...
public function view($id = 0, $result = null) {
      ...
      if($this->input->post('apprmr')){
      $query = $this->Add_material_request_model->upddata_appr($data);
             redirect('/add_material_request/');      
      }
      
      if($this->input->post('rjcmr')){
      $query = $this->Add_material_request_model->upddata_rjct($data);
             redirect('/add_material_request/');      
      } 
...
private function _get_material_info_item($mr_no) {
   ...
   private function _make_row_item($data) {
        $data1 = $this->Add_material_request_model->get_all_where(array('mr_no' => $data->mr_no))->row();
        $status_created = $data1->status_id;
        ...
        $row_data[] = "<i class='fa fa-check' aria-hidden='true'></i>";
        return $row_data;
        }
    }
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Add_material_request_model->get_details($options)->result();
    ...
    private function _make_row($data) {
        ...
        ($status_warna == "Approved"){
            $status_warna = "<span class='label label-success'>" . $status_warna . "</span>";
        }if
        ($status_warna == "Rejected"){
            $status_warna = "<span class='label label-danger'>" . $status_warna . "</span>";
        }
        $row_data = array(
            $data->date_request,
